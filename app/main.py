    ...
    if config.get('use_optimizer', False):
        ...
        try:
            ...
            if n_synthetic_samples <= 0:
                ...
                # --- Generate synthetic data in chunks to manage memory ---
                max_chunk_size = config.get('generator_max_chunk_size', 10000) # Default to 10k, configurable
                print(f"INFO: Generating {n_synthetic_samples} synthetic samples in chunks of max size {max_chunk_size}.")
                
                all_X_syn_raw_chunks = []
                remaining_samples = n_synthetic_samples
                generated_samples_count = 0

                while remaining_samples > 0:
                    current_chunk_size = min(remaining_samples, max_chunk_size)
                    print(f"INFO: Generating chunk of {current_chunk_size} samples. ({generated_samples_count}/{n_synthetic_samples} done so far)")

                    # Generate Z for the current chunk
                    Z_chunk = feeder_plugin.generate(current_chunk_size)
                    if Z_chunk is None or Z_chunk.shape[0] != current_chunk_size:
                        print(f"ERROR: FeederPlugin failed to generate the expected number of Z samples for a chunk. Expected {current_chunk_size}, got {Z_chunk.shape[0] if Z_chunk is not None else 'None'}. Exiting.")
                        sys.exit(1)

                    # Generate X_syn from Z_chunk
                    X_syn_raw_chunk = generator_plugin.generate(Z_chunk)
                    if X_syn_raw_chunk is None or X_syn_raw_chunk.shape[0] != current_chunk_size:
                        print(f"ERROR: GeneratorPlugin failed to generate the expected number of X_syn samples for a chunk. Expected {current_chunk_size}, got {X_syn_raw_chunk.shape[0] if X_syn_raw_chunk is not None else 'None'}. Exiting.")
                        sys.exit(1)
                    
                    all_X_syn_raw_chunks.append(X_syn_raw_chunk)
                    generated_samples_count += current_chunk_size
                    remaining_samples -= current_chunk_size
                    print(f"INFO: Chunk generated. Total samples processed so far: {generated_samples_count}")

                if not all_X_syn_raw_chunks:
                    print("ERROR: No synthetic data chunks were generated. Exiting.")
                    sys.exit(1)
                
                X_syn_raw_preprocessed_space = np.concatenate(all_X_syn_raw_chunks, axis=0)
