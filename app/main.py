    ...
    if config.get('use_optimizer', False):
        ...
        try:
            ...
            if n_synthetic_samples <= 0:
                ...
                # Ensure raw generator output matches the count of preprocessed feature names ('eval_feature_names')
                if X_syn_raw_preprocessed_space.shape[1] != len(eval_feature_names):
                    print(f"ERROR: Generator output {X_syn_raw_preprocessed_space.shape[1]} features, but expected {len(eval_feature_names)} preprocessed features (from eval_feature_names: {eval_feature_names}).")
                    sys.exit(1)
                ...
                # Ensure all output_feature_names are present in eval_feature_names
                ...
                if missing_original_in_preprocessed:
                    print(f"ERROR: The following original output features are not found in the preprocessed feature list (eval_feature_names): {missing_original_in_preprocessed}. Cannot select them from generated data.")
                    sys.exit(1)
                ...
                X_syn_for_output_final = X_syn_selected_original_scaled # Default to scaled if inverse transform fails or not available
                try:
                    if hasattr(preprocessor_plugin, 'inverse_transform_synthetic_data'):
                        ...
                        X_syn_for_output_final = preprocessor_plugin.inverse_transform_synthetic_data(
                            X_syn_selected_original_scaled, # Pass the (n_samples, num_original_features) array
                            datasets,
                            target_features=output_feature_names # Pass the names of the features being transformed
                        )
                        ...
                        if X_syn_for_output_final.shape[1] != len(output_feature_names):
                            ...
                            X_syn_for_output_final = X_syn_selected_original_scaled # Revert
                    else:
                    ...
                    X_syn_for_output_final = X_syn_selected_original_scaled # Revert on error
